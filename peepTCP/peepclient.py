...
from .peepserver import PEEPpacket
from netsec_2017.Lab_3.PLS import client, clientfactory
...
class PEEPClient(StackingProtocol):
    ...
    def sendack(self, ackno):
        ...
        print("Sent ACK with Ack. No.", ack.Acknowledgement)
        # For debugging
    ...
    def encapsulating_packet(self,data_from_BL_1):
        ...
        if chunk == b'rip':
            if self.called_once == 0:
                ...
                print("Sent RIP to server with Seq. No. :",self.rip.SequenceNumber,"and Ack. No. :",self.rip.Acknowledgement)
            ...
            print("Data packet sent with Seq. No.", self.Cencap.SequenceNumber, "and Ack. No.",self.Cencap.Acknowledgement, "with size of", len(chunk), "bytes")
    ...
    def pop_sending_window(self, AckNum):
        ...
        for key in self.keylist1:
            if (self.AckNum > key):
                ...
                if self.sending_window_count <= 100 and self.sending_window_count !=0 :
                    if self.backlog_window != []:
                        data_from_BL = self.backlog_window.pop(0)
                        self.encapsulating_packet(data_from_BL)
    ...
    def close(self):
        self.FLAG_CLOSE = 1
        self.pop_sending_window(self.global_number_ack)
